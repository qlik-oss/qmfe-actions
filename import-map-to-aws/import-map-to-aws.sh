...
s3_base="s3://$AWS_BUCKET_NAME/$S3_KEY" # -> s3://s3-bucket/qmfe
aws_s3_bucket_import_map_url="$s3_base/$FILE_NAME" # -> s3://s3-bucket/qmfe/import-map.json
aws_s3_metrics_url="$s3_base/metrics" # -> s3://s3-bucket/qmfe/import-map.json
invalidation_path="/$S3_KEY/$FILE_NAME" # -> /qmfe/import-map.json
aws_profile="qcs-cdn"
history_folder="${FILE_NAME%.json}-history"
...
file_exist="$(aws s3 ls "$aws_s3_bucket_import_map_url" --recursive --summarize --profile "$aws_profile" --region $AWS_REGION | grep 'Total Objects: ' | sed 's/[^0-9]*//g')"
locked="false"
